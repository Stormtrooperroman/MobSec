...
class PermissionsModule(StaticModule):
    def __init__(self):
        super().__init__("permissions_module")
    ...
    async def process(self, task_data: Dict[str, Any]) -> Dict[str, Any]:
        try:
            ...
            if task_data.get("file_type") != "apk":
                logger.error(
                    f"Invalid file type: {task_data.get('file_type')} for file: {file_path}"
                )
                return {
                    "status": "error",
            ...
            try:
                ...
                logger.info(
                    f"Found {len(dangerous)} dangerous permissions and {len(custom)} custom permissions"
                )

                return {"status": "success", "results": permissions}
