...
class ExternalModuleRegistry:
    ...
    async def register_module(self, module_data: dict) -> dict:
        ...
        if not settings.EXTERNAL_MODULES_ENABLED:
            raise HTTPException(status_code=403, detail="External modules support is disabled")
        ...
        for attempt in range(max_retries):
            ...
            if attempt < max_retries - 1:
                logger.info(f"Health check failed for {module_id}, attempt {attempt + 1}/{max_retries}. Retrying in {retry_delay} seconds...")
        ...
        logger.info(f"Module {module_id} {'registered successfully' if is_healthy else 'registered with errors'}")
        return module_dict
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                try:
                    ...
                    if response.status_code == 200:
                        logger.info(f"Module {health_url} is healthy. Status: {response.status_code}")
                        return True
                    else:
                        logger.warning(f"Module health check failed {health_url}. Status: {response.status_code}, Response: {response.text}")
                        return False
                except httpx.ConnectError as e:
                    logger.warning(f"Connection error to module {health_url}: {e.__class__.__name__} - {str(e)}")
                    return False
                except httpx.TimeoutException as e:
                    logger.warning(f"Timeout connecting to module {health_url}: {e.__class__.__name__} - {str(e)}")
                    return False
        ...
        while True:
            try:
                ...
                for db_module in db_modules:
                    time_since_last_heartbeat = (datetime.now(timezone.utc) - db_module.last_heartbeat).total_seconds()
                    if time_since_last_heartbeat > self.health_check_interval:
                        ...
                        async with AsyncSessionLocal() as session:
                            result = await session.execute(
                                select(ExternalModule).where(ExternalModule.module_id == db_module.module_id)
                            )
                            module = result.scalars().first()
                            
                            if module:
                                old_status = module.status
                                module.status = ModuleStatus.ACTIVE if is_healthy else ModuleStatus.ERROR
                                ...
                                if old_status != module.status:
                                    logger.info(f"Module {db_module.module_id} status changed from {old_status} to {module.status}")
                        
                        logger.debug(f"Health check for module {db_module.module_id}: {'healthy' if is_healthy else 'unhealthy'}")
