...
async def update_auto_run_settings(settings: Dict[str, Any]):
    ...
    required_keys = ["zip_action", "zip_action_type", "apk_action", "apk_action_type", "ipa_action", "ipa_action_type"]
    ...
    for action_type_key in ["zip_action_type", "apk_action_type", "ipa_action_type"]:
        if settings[action_type_key] not in valid_action_types:
            raise HTTPException(status_code=400, detail=f"Invalid {action_type_key}. Must be one of: {valid_action_types}")
