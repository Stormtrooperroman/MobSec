...
class EmulatorManager:
    ...
    async def _wait_for_android_boot(self, host: str, port: int, timeout: int = 120) -> bool:
        ...
        while (asyncio.get_event_loop().time() - start_time) < timeout:
            try:
                ...
                if devices_result.returncode == 0 and f"{host}:{port}" in devices_result.stdout:
                    boot_check = subprocess.run(
                        ["adb", "-s", f"{host}:{port}", "shell", "getprop", "sys.boot_completed"], 
                        capture_output=True, 
                        text=True, 
                        timeout=10,
                        env=env
    ...
    async def _adb_connect(self, host: str, port: int) -> bool:
        ...
        try:
            ...
            success = result.returncode == 0 or "connected to" in result.stdout.lower() or "already connected" in result.stdout.lower()
            if success:
    ...
    async def _register_emulator(self, emulator_name: str, config: dict):
        ...
        async with self.async_session() as session:
            try:
                ...
                version = str(config.get('version', config.get('android_version', '12.0.0')))
    ...
    async def start_emulator(self, emulator_name: str) -> Dict[str, Any]:
        ...
        return {"name": emulator_name, "status": "running", "container_id": container.id}
        ...
        try:
            async with self.async_session() as session:
                ...
                if emulator and emulator.container_id:
                    try:
                        container = self.docker_client.containers.get(emulator.container_id)
                        container.stop()
                        container.remove()
    ...
    async def _update_emulator_status(self, emulator_name: str, status: str, container_id: str = None):
    ...
    async def start_active_emulators(self):
        ...
        try:
            ...
            active_emulators = [name for name, config in self.emulators_config.items() 
            ...
            if active_emulators:
                ...
                logger.info(f"Started {started}/{len(active_emulators)} emulators successfully")
            else:
