    ...
    async def _process_result(self, result_key: str):
        ...
        try:
            ...
            try:
                ...
                logger.error(
                    f"Failed to parse JSON result for {result_key}: {result_json[:100]}..."
                )
                return
        ...
        try:
            ...
            if not isinstance(file_info, dict) or file_info.get("scan_results") is None:
                ...
                if not isinstance(current_results, dict):
                    logger.warning(
                        f"scan_results is not a dictionary, got {type(current_results)}"
                    )
                    current_results = {}
            ...
            if success:
                logger.info(
                    f"Updated scan results for file {file_hash} with {module_name} results"
                )
            else:
                ...
                if chain_data.get("file_hash") == file_hash:
                    ...
                    if (
                        ...
                        self.redis_client.set(
                            chain_key, json.dumps(chain_data), ex=86400
                        )
                        ...
                        logger.info(
                            f"Published chain module completion event for {module_name} in chain {chain_task_id}"
                        )
