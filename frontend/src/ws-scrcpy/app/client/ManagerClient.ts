...
export abstract class ManagerClient<P extends ParamsBase, TE extends EventMap> extends BaseClient<P, TE> {
  ...
  protected wrapInProxy(directUrl: URL): URL {
    ...
    const localUrl = new URL(`${localProtocol}//${location.host}${this.params.pathname}`);
    localUrl.searchParams.set('action', 'stream');
    return localUrl;
  }
  ...
  protected getChannelInitData(): Uint8Array {
    ...
    return encoder.encode(ManagerClient.CODE);
  }
