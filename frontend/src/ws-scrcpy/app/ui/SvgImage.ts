...
export default class SvgImage {
  ...
  public static create(type: Icon): Element {
    try {
      const svgString = this.getSvgString(type);
      if (!svgString) {
        console.warn(`No SVG found for icon type: ${type}`);
      ...
      if (!svg) {
        console.warn(`Failed to create SVG element for icon type: ${type}`);
        return document.createElement('span');
      }
      ...
      console.error(`Error creating SVG for icon type ${type}:`, error);
      return document.createElement('span');
    }
  }
}
